...
import BalloonPop from "../balloonPop/balloonPop"

const GameJoinMobile = () => {
    const [gameList, setGameList] = useState([])
    const { currentUser } = useAuth()
    const history = useHistory()

    function getGames() {
        fetch("http://137.117.166.239:5000/get_games_mobile/")
    ...
    function createGame() {
        ...
        fetch("http://137.117.166.239:5000/create_game_mobile/", requestParams)
        history.push("/balloonMulti")
    }
    ...
    function buttonClicked(creator) {
        ...
        fetch("http://137.117.166.239:5000/game_join_mobile/", requestParams)
        history.push("/balloonMulti")
    }
